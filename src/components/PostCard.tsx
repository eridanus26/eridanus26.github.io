  ...
  return (
    <div className="h-full">
      <motion.div
        ...
        className="collage-card group h-full flex flex-col relative"
      >
        {/* Main background link to the post detail page */}
        <Link to={`/post/${post.id}`} className="absolute inset-0 z-0" />
        ...
        <div className="p-8 pt-4 flex-1 flex flex-col space-y-6 z-10 pointer-events-none">
          <div className="flex items-center justify-between text-base uppercase tracking-[0.2em] text-[#9A7A78] font-serif">
            ...
            {/* Location block un-frozen with pointer-events-auto */}
            {post.location && (
              <span className="flex items-center gap-2 pointer-events-auto relative z-20"> 
                <MapPin size={14} className="text-[#A84848]/40" />
                <a
